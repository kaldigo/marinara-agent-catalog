...
Better Impersonate has no per-chat agent entry or replacement settings page. Its dry runs use Marinara's native global Impersonate prompt, connection/model, preset, and agent-blocking settings. The package adds only small mode-specific wrappers for Draft, Continue, and Private Thinking. Mari Bridge resolves the Quick Reply `{{input}}` macro before dispatching the slash command.
...
Stop button. Better Impersonate owns only the additional commands, their small
guidance wrappers, last-guidance recall, and Quick Reply dispatch. It must not
