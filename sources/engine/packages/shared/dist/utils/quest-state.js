...
const NESTED_OBJECTIVE_KEYS = ["objectives", "tasks", "steps", "items", "subtasks", "children", "goals"];
const NESTED_QUEST_KEYS = ["quests", "activeQuests", "groups", "items", "children"];
...
function normalizeQuestAction(value) {
    if (typeof value !== "string")
        return null;
...
function collectNestedObjectives(value, depth) {
    const nested = [];
...
function collectObjectives(value, depth = 0) {
...
function hasQuestProgressFields(value) {
    return (value.questEntryId !== undefined ||
        value.objectives !== undefined ||
        value.currentStage !== undefined ||
        value.completed !== undefined);
}
function normalizeQuestProgress(value, fallbackName) {
    if (!isRecord(value))
        return null;
...
function collectNestedQuests(value, depth) {
    const nested = [];
...
export function normalizeQuestCollectionForQuestMerge(value, depth = 0) {
    if (value == null || depth > 5)
        return [];
    if (Array.isArray(value)) {
...
function normalizeQuestUpdate(value) {
    if (!isRecord(value))
        return null;
...
export function normalizeQuestUpdates(value) {
    if (!Array.isArray(value))
        return [];
...
export function normalizePlayerStatsForQuestMerge(value) {
...
export function compactQuestProgressForContext(value) {
...
export function applyQuestUpdatesToPlayerStats(value, updatesValue, options = {}) {
        ...
        if (update.action === "create" && idx === -1) {
            ...
            if (update.action === "update") {
                if (update.objectives !== undefined)
                    quests[idx].objectives = update.objectives;
            }
...
export function buildQuestJournalData(update) {
    return {
        id: update.questName,
        name: update.questName,
...
//# sourceMappingURL=quest-state.js.map