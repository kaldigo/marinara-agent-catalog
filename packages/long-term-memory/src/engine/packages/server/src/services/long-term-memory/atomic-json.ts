...
export async function fsyncPath(path: string) {
  let h;
  try {
    h = await open(path, constants.O_RDONLY);
    await h.sync();
  } catch {
  } finally {
    await h?.close().catch(() => {});
...
async function atomic(path: string, content: string) {
  ...
  let h;
  try {
    h = await open(tmp, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY, 0o600);
...
export function writeJsonAtomic(path: string, value: unknown) {
  return atomic(path, `${JSON.stringify(value, null, 2)}\n`);
}
...
export async function appendJsonLineAtomic(path: string, value: unknown) {
  await mkdir(dirname(path), { recursive: true });
  const h = await open(path, constants.O_CREAT | constants.O_APPEND | constants.O_WRONLY, 0o600);
  try {
