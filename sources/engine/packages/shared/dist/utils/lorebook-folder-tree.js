 ...
 */
export function canReparentFolder(folders, folderId, newParentId) {
    ...
    if (!newParent)
        return { ok: false, reason: "Target parent folder not found." };
...
export function collectHiddenFolderIds(folders, collapsedFolderIds) {
 ...
 */
export function collectFolderSubtreeIds(folders, rootId) {
    const childrenByParent = new Map();
    for (const folder of folders) {
        const parentId = folder.parentFolderId;
        if (!parentId)
 ...
 */
export function collectEffectivelyDisabledFolderIds(folders) {
    const byId = new Map(folders.map((folder) => [folder.id, folder]));
 ...
 */
export function buildFolderForest(folders) {
        ...
        if (parentId !== null) {
            ...
            if (siblings) {
                ...
                if (remaining.length > 0)
                    childrenByParent.set(parentId, remaining);
                else
...
//# sourceMappingURL=lorebook-folder-tree.js.map