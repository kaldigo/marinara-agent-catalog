...
export function getSpatialDescendantIds(definition, locationId) {
...
export function wouldCreateSpatialCycle(definition, locationId, parentId) {
    if (parentId === locationId)
        return true;
    return getSpatialDescendantIds(definition, locationId).has(parentId);
}
export function resolveSpatialLocationDepth(definition, location) {
    ...
    while (current.parentId) {
        if (seen.has(current.parentId))
            return SPATIAL_CONTEXT_LIMITS.maxDepth + 1;
...
export function spatialRadialPlacement(index, count, radius = count <= 6 ? 34 : 40) {
    if (count === 1)
        return { x: 50, y: 50 };
...
export function compareSpatialLocations(left, right) {
...
export function validateSpatialContextDefinition(definition) {
    const issues = [];
    const firstIndexById = new Map();
    const byId = new Map();
    if (definition.locations.length > SPATIAL_CONTEXT_LIMITS.maxLocations) {
        issues.push(issue("too_many_locations", `A spatial map can contain at most ${SPATIAL_CONTEXT_LIMITS.maxLocations} locations.`, ["locations"]));
    }
    definition.locations.forEach((location, index) => {
        const firstIndex = firstIndexById.get(location.id);
        if (firstIndex !== undefined) {
            issues.push(issue("duplicate_location_id", `Location ID "${location.id}" is already used by another location.`, ["locations", index, "id"], location.id));
            return;
        }
    ...
    if (definition.startingLocationId !== null) {
        ...
        if (!startingLocation) {
            issues.push(issue("starting_location_missing", "The starting location does not exist.", ["startingLocationId"], definition.startingLocationId));
        }
        else if (startingLocation.status !== "active") {
            issues.push(issue("starting_location_archived", "The starting location must be active.", ["startingLocationId"], startingLocation.id));
        }
    }
    definition.locations.forEach((location, index) => {
        if (location.parentId !== null) {
            if (location.parentId === location.id) {
                issues.push(issue("self_parent", "A location cannot be its own parent.", ["locations", index, "parentId"], location.id));
            }
            else if (!byId.has(location.parentId)) {
                issues.push(issue("parent_missing", "The selected parent location does not exist.", ["locations", index, "parentId"], location.id));
            }
        }
        if (location.links.length > SPATIAL_CONTEXT_LIMITS.maxLinksPerLocation) {
            issues.push(issue("too_many_links", `A location can contain at most ${SPATIAL_CONTEXT_LIMITS.maxLinksPerLocation} links.`, ["locations", index, "links"], location.id));
        }
        const seenLorebookEntryIds = new Set();
        location.lorebookEntryIds.forEach((entryId, entryIndex) => {
            if (seenLorebookEntryIds.has(entryId)) {
                issues.push(issue("duplicate_lorebook_entry_id", "A lorebook entry can be attached to a location only once.", ["locations", index, "lorebookEntryIds", entryIndex], location.id));
            }
        ...
        location.links.forEach((link, linkIndex) => {
            if (link.targetId === location.id) {
                issues.push(issue("self_link", "A location cannot link to itself.", ["locations", index, "links", linkIndex, "targetId"], location.id));
            }
            else if (!byId.has(link.targetId)) {
                issues.push(issue("link_target_missing", "The linked location does not exist.", ["locations", index, "links", linkIndex, "targetId"], location.id));
            }
            if (seenLinkTargets.has(link.targetId)) {
                issues.push(issue("duplicate_link_target", "A location can link to a destination only once.", ["locations", index, "links", linkIndex, "targetId"], location.id));
            }
    ...
    definition.locations.forEach((location, index) => {
        ...
        if (cycleFound) {
            issues.push(issue("parent_cycle", "Location parents must not form a cycle.", ["locations", index, "parentId"], location.id));
        }
        else if (depth > SPATIAL_CONTEXT_LIMITS.maxDepth) {
            issues.push(issue("maximum_depth_exceeded", `Location nesting cannot exceed ${SPATIAL_CONTEXT_LIMITS.maxDepth} levels.`, ["locations", index, "parentId"], location.id));
        }
    });
    ...
    definition.locations.forEach((location, index) => {
        ...
        if (children)
            ...
            childrenByParent.set(location.parentId, [entry]);
    });
    for (const [parentId, children] of childrenByParent) {
        const parent = byId.get(parentId);
        if (!parent || parent.childPresentation !== "layers")
            continue;
        const usedOrders = new Map();
        for (const { location, index } of children) {
            if (location.layerOrder === undefined) {
                issues.push(issue("layer_order_missing", "Every child of a layer location needs a layer order.", ["locations", index, "layerOrder"], location.id));
                continue;
            }
            const existingLocationId = usedOrders.get(location.layerOrder);
            if (existingLocationId !== undefined) {
                issues.push(issue("duplicate_layer_order", `Layer order ${location.layerOrder} is already used by location "${existingLocationId}".`, ["locations", index, "layerOrder"], location.id));
            }
            else {
...
export function resolveSpatialBreadcrumb(definition, locationId) {
    if (locationId === null)
        return [];
    const byId = buildSpatialLocationIndex(definition);
    const breadcrumb = [];
...
export function resolveSpatialDestinations(definition, currentLocationId) {
    if (!definition.enabled || currentLocationId === null)
        return [];
    const byId = buildSpatialLocationIndex(definition);
    ...
    const add = (location, relation, label) => {
        if (!location || location.status !== "active" || destinations.has(location.id))
            return;
        destinations.set(location.id, destinationFromLocation(location, relation, label));
    };
    if (current.parentId !== null)
        add(byId.get(current.parentId), "leave");
    for (const location of definition.locations) {
        if (location.parentId === current.id)
            add(location, "enter");
    }
    for (const link of current.links) {
        if (link.state === "available")
            add(byId.get(link.targetId), "link", link.label);
    }
...
export function validateSpatialArchive(definition, locationId, options) {
    const byId = buildSpatialLocationIndex(definition);
    if (!byId.has(locationId)) {
        return { ok: false, code: "spatial_location_missing", message: "The location does not exist." };
    }
    if (definition.startingLocationId === locationId &&
...
export function validateSpatialTransition(definition, currentLocationId, request) {
