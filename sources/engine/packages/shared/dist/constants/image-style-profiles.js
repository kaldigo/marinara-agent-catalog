...
export const DEFAULT_IMAGE_STYLE_PROFILES = [
    {
        id: "off",
        name: "Off",
        baseStyle: "custom",
    ...
    },
    {
        ...
        baseStyle: "anime",
        promptMode: "tagged",
        styleText: "Anime illustration with clean character design, expressive faces, crisp linework, and polished lighting.",
    ...
    },
    {
        ...
        negativeTags: "worst quality, low quality, lowres, bad anatomy, bad hands, extra digits, fewer digits, text, watermark, logo, signature",
        subjectTags: {
            portrait: "solo, portrait, upper body, looking at viewer",
            avatar: "solo, portrait, upper body, centered composition",
            selfie: "solo, selfie, close-up, looking at viewer",
    ...
    },
    {
        ...
        baseStyle: "realistic",
        promptMode: "natural",
        styleText: "Realistic SDXL-style image with natural lighting, believable materials, lens-aware composition, and sharp detail.",
    ...
    },
    {
        ...
        promptMode: "natural",
        styleText: "Photorealistic SDXL image with believable skin, optics, materials, camera framing, and natural scene lighting.",
        positiveTags: "photorealistic, high quality, sharp focus, natural lighting, detailed textures",
        negativeTags: "anime, cartoon, illustration, painting, plastic skin, uncanny face, low quality, blurry, text, watermark, logo, signature",
        subjectTags: {
    ...
    },
    {
        ...
        baseStyle: "cinematic",
        promptMode: "hybrid",
        styleText: "Cinematic key art with controlled lighting, strong composition, atmospheric depth, and emotionally clear staging.",
    ...
    },
    {
        ...
        promptMode: "natural",
        styleText: "Z-Image Turbo prompt that keeps compact narrative expression, coherent subjects, clear composition, and natural visual intent.",
        positiveTags: "",
...
export function normalizeImageStyleProfile(raw) {
    if (!isRecord(raw))
        return null;
    ...
    return {
        ...
        baseStyle: readEnum(raw.baseStyle, [
            "auto",
            "anime",
            "danbooru",
            "realistic",
...
export function suggestImageStyleProfileIdForModel(model, source, service) {
    const haystack = [model, source, service].filter(Boolean).join(" ").toLowerCase();
    if (!haystack.trim())
        return null;
    if (/(?:z[-_\s]?image|zit|z[-_\s]?image[-_\s]?turbo)/.test(haystack))
        return "z-image-turbo";
    if (/(?:illustrious|pony|noobai|novelai|nai|danbooru|e621|animagine|kohaku)/.test(haystack))
        return "danbooru";
    if (/(?:realvis|juggernaut|epicrealism|realistic|photoreal|photo|albedo|cyberrealistic)/.test(haystack)) {
        return "photorealistic";
    }
    if (/(?:dreamshaper|cinematic|film|movie|still)/.test(haystack))
        return "cinematic";
    if (/(?:digital[-_\s]?painting|concept[-_\s]?art|paint|illustration)/.test(haystack))
        return "digital-painting";
    if (/(?:anime|anything|counterfeit|meinahentai|meinamix|abyssorangemix|waifu|manga)/.test(haystack))
        return "anime";
    if (/(?:sdxl|stable[-_\s]?diffusion[-_\s]?xl|xl)/.test(haystack))
        return "realistic";
    return null;
}
export function findImageStyleProfile(settings, profileId) {
...
function normalizeSubjectTags(raw, fallback = {}) {
    const result = {};
    for (const kind of ["portrait", "selfie", "background", "illustration", "sprite", "avatar"]) {
        result[kind] = readString(raw[kind], fallback[kind] ?? "").slice(0, 1000);
    }
    return result;
}
function normalizeRules(raw, fallback) {
...
function readEnum(value, values, fallback) {
    return typeof value === "string" && values.includes(value) ? value : fallback;
}
//# sourceMappingURL=image-style-profiles.js.map