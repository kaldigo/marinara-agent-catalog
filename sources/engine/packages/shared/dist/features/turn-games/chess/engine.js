...
import { CHESS_LOG_CAP, CHESS_MAX_PLAYERS, CHESS_MIN_PLAYERS, DEFAULT_CHESS_CONFIG, chessConfigSchema, } from "./types.js";
// ── small helpers ───────────────────────────────────────────────────────────
...
function seatForColor(state, color) {
    return color === "w" ? state.whiteSeatId : state.blackSeatId;
}
function colorOfSeat(state, seatId) {
    if (seatId === state.whiteSeatId)
        return "white";
    if (seatId === state.blackSeatId)
        return "black";
    return null;
}
function record(state, event) {
    state.log.push(event);
    if (state.log.length > CHESS_LOG_CAP)
        state.log.splice(0, state.log.length - CHESS_LOG_CAP);
}
...
function normalizeSan(raw) {
    ...
    san = san.replace(/^\d+\.+\s*/, "");
    san = san.replace(/[!?]+$/, "");
    if (/^[0o]-[0o]-[0o]/i.test(san))
        san = `O-O-O${san.slice(5)}`;
    else if (/^[0o]-[0o]/i.test(san))
        san = `O-O${san.slice(3)}`;
    return san.trim();
}
function toChessMove(m) {
    return {
        type: "move",
        san: m.san,
        from: m.from,
...
function recentMovesLine(state, maxPlies) {
...
function describeAttempt(move) {
    if (move.san && move.san.trim())
        return move.san.trim();
    if (move.from && move.to)
        return `${move.from}-${move.to}${move.promotion ? `=${move.promotion.toUpperCase()}` : ""}`;
    return "(no move given)";
}
function moveFlavor(state, seatId, made) {
    const name = nameOf(state, seatId);
    if (made.flags.includes("k"))
        return `${name} castles kingside (O-O).`;
    if (made.flags.includes("q"))
        return `${name} castles queenside (O-O-O).`;
    let text = `${name} plays ${made.san}`;
    if (made.captured) {
        text += `, capturing a ${pieceName(made.captured)}`;
        if (made.flags.includes("e"))
            text += " en passant";
    }
    if (made.promotion)
        text += `, promoting to a ${pieceName(made.promotion)}`;
    return `${text}.`;
}
// ── prompt builders ─────────────────────────────────────────────────────────
function buildBoardSummary(state, seatId, c) {
    ...
    const lines = [];
    lines.push(`Chess — move ${moveNumberFromFen(state.fen)}. You are ${nameOf(state, seatId)}, playing ${yourColor}. ` +
    ...
    if (recent)
        lines.push(`Recent moves: ${recent}`);
    if (state.check)
        lines.push("You are IN CHECK — your move must get your king out of check.");
    return lines.join("\n");
}
function buildInstructions(sanList) {
    return (`Legal moves (SAN): ${sanList.join(", ")}\n` +
...
function buildSpectatorSummary(state) {
    ...
    if (state.status === "finished") {
        ...
        if (state.winnerSeatId) {
            ...
            lines.push(`The chess game between ${white} (white) and ${black} (black) just ended in a draw by ${drawReasonText(state.drawReason)}.`);
        }
        else {
    ...
    if (recent)
        lines.push(`Recent moves: ${recent}`);
    if (state.lastMove)
        lines.push(`Last move: ${nameOf(state, state.lastMove.seatId)} played ${state.lastMove.san}.`);
...
function buildParticipantSummary(state, seatId) {
    ...
    if (state.lastMove?.seatId === seatId)
        lines.push(`Your last move was ${state.lastMove.san}.`);
    return lines.join("\n");
}
...
function pickFallback(state) {
    ...
    if (bestCapture)
        return toChessMove(bestCapture);
    const check = moves.find((m) => m.san.includes("+"));
    if (check)
        return toChessMove(check);
...
export const chessEngine = {
    ...
    normalizeConfig(config) {
        ...
        if (direct.success)
            return direct.data;
        const merged = { ...DEFAULT_CHESS_CONFIG, ...(config && typeof config === "object" ? config : {}) };
    ...
    setup(config, seats, seed) {
        ...
        const humanIsWhite = config.humanColor === "white" ? true : config.humanColor === "black" ? false : Math.abs(seed) % 2 === 0;
    ...
    publicView(state, viewerSeatId) {
        ...
        const legalMovesForYou = viewerSeatId && viewerSeatId === currentSeatId
    ...
    parseToolCall(name, args) {
        ...
        const promotion = promotionRaw && ["q", "r", "b", "n"].includes(promotionRaw.charAt(0))
            ? promotionRaw.charAt(0)
            : undefined;
