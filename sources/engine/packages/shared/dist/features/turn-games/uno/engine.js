...
import { DEFAULT_UNO_CONFIG, UNO_LOG_CAP, UNO_MAX_PLAYERS, UNO_MIN_PLAYERS, unoConfigSchema, } from "./types.js";
// ── small helpers ───────────────────────────────────────────────────────────
...
function findHandCard(hand, cardId, face) {
    if (cardId)
        return hand.find((c) => c.id === cardId) ?? null;
    if (face)
        return hand.find((c) => c.color === face.color && c.value === face.value) ?? null;
    return null;
}
/** Playable ignoring any pending draw penalty: matches active color, top value, or is a wild. */
function isNormallyPlayable(card, state) {
...
function isStackable(card, state) {
    if (state.pendingDraw <= 0 || !state.config.stacking)
        return false;
    if (state.pendingDrawType === "draw2")
        return card.value === "draw2";
    if (state.pendingDrawType === "wild4")
        return card.value === "wild4";
    return false;
}
function record(state, event) {
    state.log.push(event);
    if (state.log.length > UNO_LOG_CAP)
        state.log.splice(0, state.log.length - UNO_LOG_CAP);
}
...
function applyZeroRotate(state, events) {
...
function applyCardEffectAndAdvance(state, seatId, card, opts, events) {
    switch (card.value) {
        case "skip": {
        ...
        case "7": {
            if (state.config.sevenZero && opts.swapTargetSeatId)
                applySevenSwap(state, seatId, opts.swapTargetSeatId, events);
            advance(state, 1);
            break;
        }
        case "0": {
            if (state.config.sevenZero)
                applyZeroRotate(state, events);
            advance(state, 1);
            break;
        }
        default: {
...
function applyCardPlay(state, seatId, card, opts, events) {
    const hand = state.hands[seatId];
    if (!hand)
        return fail("You are not seated in this game.");
    const idx = hand.findIndex((c) => c.id === card.id);
    if (idx < 0)
        return fail("That card isn't in your hand.");
    hand.splice(idx, 1);
    state.discardPile.push(card);
    const isWild = card.color === "wild";
    const newColor = isWild ? opts.declaredColor : card.color;
...
function resolvePlay(state, seatId, card, move, events) {
    const hand = state.hands[seatId];
    if (!hand)
        return fail("You are not seated in this game.");
    const isWild = card.color === "wild";
    ...
    return applyCardPlay(state, seatId, card, { declaredColor: move.declaredColor, sayUno: move.sayUno, swapTargetSeatId }, events);
}
// ── per-move handlers (operate on the already-cloned state) ───────────────────
function handlePlay(state, seatId, move, events) {
    const hand = state.hands[seatId];
    if (!hand)
        return fail("You are not seated in this game.");
    if (state.status === "awaiting_post_draw") {
        ...
        const matchesDrawn = !!drawn &&
            (move.cardId
                ? move.cardId === drawn.id
                : !move.card || (move.card.color === drawn.color && move.card.value === drawn.value));
        if (!drawn || !matchesDrawn)
            return fail("After drawing you may only play the card you drew, or pass.");
        state.status = "awaiting_move";
        state.drawnCardId = null;
...
function handlePlayDrawn(state, seatId, move, events) {
    ...
    if (!card)
        return fail("The drawn card is no longer in your hand.");
    if (!isNormallyPlayable(card, state))
        return fail("The drawn card isn't playable.");
    state.status = "awaiting_move";
    state.drawnCardId = null;
    return resolvePlay(state, seatId, card, move, events);
}
function handleDraw(state, seatId, events) {
    // Calling draw while a penalty is pending resolves the penalty.
    if (state.pendingDraw > 0)
        return handleDrawPenalty(state, seatId, events);
    let lastDrawn = null;
    let playable = false;
...
function handlePass(state, _seatId, events) {
    if (state.status !== "awaiting_post_draw")
        return fail("There's nothing to pass on — play or draw.");
...
function handleDrawPenalty(state, seatId, events) {
    if (state.pendingDraw <= 0)
        return fail("There is no draw penalty to take.");
...
function handleJumpIn(state, seatId, move, events) {
    if (!state.config.jumpIn)
        return fail("The jump-in rule is off.");
    if (state.pendingDraw > 0)
        return fail("You can't jump in while a draw penalty is pending.");
    const hand = state.hands[seatId];
    if (!hand)
        return fail("You are not seated in this game.");
    const top = topCard(state);
    if (!top)
        return fail("No card to jump on.");
    const card = findHandCard(hand, move.cardId, move.card);
    if (!card)
        return fail("That card isn't in your hand.");
...
function handleCallOut(state, seatId, move, events) {
    const target = move.targetSeatId;
    if (target === seatId)
        return fail("You can't catch yourself.");
    if (!state.seatOrder.includes(target))
...
function handleDeclareUno(state, seatId, events) {
    if ((state.hands[seatId]?.length ?? 0) !== 1)
        return fail("You can only call UNO when you hold exactly one card.");
    state.mustCallUno[seatId] = false;
    events.push({ type: "uno_called", seatId, message: `${nameOf(state, seatId)} calls UNO!` });
    return { ok: true, state, events };
}
...
function pushCatchMoves(state, seatId, moves) {
...
function expandPlay(state, seatId, card, kind) {
    const out = [];
    const isWild = card.color === "wild";
    const willWin = (state.hands[seatId]?.length ?? 0) === 1;
    const needsSwap = state.config.sevenZero && card.value === "7" && !willWin;
    const colorVariants = isWild ? [...ALL_COLORS] : [undefined];
    const swapTargets = needsSwap ? state.seatOrder.filter((s) => s !== seatId) : [undefined];
...
function enumerateLegalMoves(state, seatId) {
    ...
    if (hand.length === 1 && state.mustCallUno[seatId])
        moves.push({ type: "declare_uno" });
    if (!isCurrent) {
            ...
            if (top && top.color !== "wild") {
                for (const c of hand) {
                    if (c.color !== "wild" && c.color === top.color && c.value === top.value) {
                        ...
                        if (state.config.sevenZero && c.value === "7" && hand.length !== 1)
                            continue;
    ...
    if (state.status === "awaiting_post_draw") {
        ...
        if (drawn && drawnPlayable)
            moves.push(...expandPlay(state, seatId, drawn, "play_drawn"));
        if (!(state.config.forcePlay && drawnPlayable))
    ...
    if (state.pendingDraw > 0) {
        ...
        if (state.config.stacking) {
            for (const c of hand) {
                if (isStackable(c, state))
                    moves.push(...expandPlay(state, seatId, c, "play"));
            }
        }
    }
    else {
...
function pickFallback(state, seatId) {
    ...
    if (currentSeatId(state) !== seatId) {
        return (moves.find((m) => m.type === "call_out") ??
    ...
    if (plays.length > 0) {
        ...
        for (const move of plays) {
            ...
            if (card.color === "wild" && move.declaredColor !== dom)
                score -= 1; // prefer wild variant that declares dominant color
            if (card.color !== "wild" && card.color === state.activeColor)
                score += 0.5;
            if (score > bestScore) {
                bestScore = score;
                best = move;
...
function buildBoardSummary(state, seatId) {
    ...
    if (state.pendingDraw > 0) {
        lines.push(`A +${state.pendingDraw} penalty is stacked on you. Draw it with draw_card` +
    ...
    if (plays.length)
        lines.push("What just happened:", ...plays);
    return lines.join("\n");
}
function buildInstructions(state, seatId) {
    ...
    if (state.status === "awaiting_post_draw") {
        ...
        parts.push("Resolve the draw penalty: draw_card to take it" +
...
function recentPlayLines(state, max) {
    return state.log
...
function buildSpectatorSummary(state) {
    if (state.status === "finished") {
        ...
        if (plays.length)
            lines.push("How it played out:", ...plays);
        return lines.join("\n");
    }
    const top = topCard(state);
    const lines = ["A game of UNO is in progress at the table."];
    lines.push(`Top card: ${top ? cardLabel(top) : "—"} (active color: ${cap(state.activeColor)}). ` +
...
function buildParticipantSummary(state, seatId) {
    ...
    if (state.status !== "finished") {
        ...
        if (currentSeatId(state) === seatId)
            lines.push("It is YOUR turn to act.");
    }
    const lastOwn = [...state.log]
        .reverse()
        .find((e) => e.seatId === seatId && e.type !== "deal" && typeof e.message === "string" && e.message.trim().length > 0);
    if (lastOwn)
        lines.push(`Your most recent action: ${lastOwn.message}`);
    return lines.join("\n");
}
// ── the engine object ──────────────────────────────────────────────────────────
export const unoEngine = {
    gameType: "uno",
    schemaVersion: 1,
    label: "UNO",
    ...
    normalizeConfig(config) {
        ...
        if (direct.success)
            return direct.data;
        const merged = { ...DEFAULT_UNO_CONFIG, ...(config && typeof config === "object" ? config : {}) };
    ...
    setup(config, seats, seed) {
        ...
        if (!startCard)
            startCard = { id: "fallback-red-0", color: "red", value: "0" };
        const state = {
            config,
            seatOrder,
            seatNames,
    ...
    interruptibleSeats(state) {
        ...
        for (const s of state.seatOrder) {
            if (s === cur)
                continue;
            if (enumerateLegalMoves(state, s).some((m) => m.type === "jump_in" || m.type === "call_out"))
                out.push(s);
        }
        return out;
    },
    ...
    describeForModel(state, seatId) {
        ...
        const legalMoves = enumerateLegalMoves(state, seatId).map((m) => m.type === "draw_penalty" ? { type: "draw" } : m);
        return {
    ...
    publicView(state, viewerSeatId) {
        ...
        if (viewerSeatId && state.hands[viewerSeatId]) {
            ...
            for (const m of moves) {
                if (m.type === "play" && m.cardId)
                    playableIds.add(m.cardId);
                else if (m.type === "play_drawn")
                    drawnPlayable = true;
                else if (m.type === "jump_in" && m.cardId)
                    jumpInSet.add(m.cardId);
                else if (m.type === "call_out")
                    catchSet.add(m.targetSeatId);
                else if (m.type === "draw")
                    yourActions.canDraw = true;
                else if (m.type === "draw_penalty")
                    yourActions.canDrawPenalty = true;
                else if (m.type === "pass")
                    yourActions.canPass = true;
            }
