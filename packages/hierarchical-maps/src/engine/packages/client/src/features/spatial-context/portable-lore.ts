import type { Lorebook, LorebookEntry, LorebookFolder, SpatialContextDefinition } from "@marinara-engine/shared";

export type PortableLoreExportMode = "map-only" | "linked-entries" | "complete-lorebooks";
...
export function nextPortableLorebookName(
  ...
  const reserved = new Set([...reservedNames].map((name) => name.trim().toLocaleLowerCase()));
...
export function portableEntryData(entry: LorebookEntry): Record<string, unknown> {
  return {
    name: entry.name,
    ...pickRecord(entry as unknown as Record<string, unknown>, ENTRY_SETTING_KEYS),
  };
}

export function portableEntryFingerprint(data: Record<string, unknown>): string {
  return `entry-v1:${shortHash(stableStringify(data))}`;
}

function samePortableEntry(left: Record<string, unknown>, right: Record<string, unknown>): boolean {
  return stableStringify(left) === stableStringify(right);
}

function requiredFolderIds(entries: LorebookEntry[], folders: LorebookFolder[]): Set<string> {
...
export function buildPortableLoreBundle(options: {
  ...
  const lorebooksById = new Map(options.lorebooks.map((book) => [book.id, book]));
  const entriesById = new Map(options.entries.map((entry) => [entry.id, entry]));
  const linkedEntries = Array.from(
    new Set(options.definition.locations.flatMap((location) => location.lorebookEntryIds)),
  )
  ...
  const bookKeyById = new Map(selectedBooks.map((book, index) => [book.id, `book-${index + 1}`]));
  const selectedEntries = selectedBooks.flatMap((book) => {
    const bookEntries = options.entries.filter((entry) => entry.lorebookId === book.id);
    return (
      options.mode === "complete-lorebooks" ? bookEntries : bookEntries.filter((entry) => linkedEntryIds.has(entry.id))
    ).sort((left, right) => left.id.localeCompare(right.id));
  });
  const entryKeyById = new Map(selectedEntries.map((entry, index) => [entry.id, `entry-${index + 1}`]));

  const books =
    options.mode === "map-only"
      ? []
      : selectedBooks.map((book): PortableLoreBook => {
          const bookEntries = selectedEntries.filter((entry) => entry.lorebookId === book.id);
          ...
          const includedFolders = allFolders
            ...
            .sort((left, right) => left.id.localeCompare(right.id));
          const folderKeyById = new Map(includedFolders.map((folder, index) => [folder.id, `folder-${index + 1}`]));
          return {
            key: bookKeyById.get(book.id)!,
            originalId: book.id,
            name: book.name,
            settings: pickRecord(book as unknown as Record<string, unknown>, LOREBOOK_SETTING_KEYS),
            folders: includedFolders.map((folder) => ({
              key: folderKeyById.get(folder.id)!,
              originalId: folder.id,
              parentKey: folder.parentFolderId ? (folderKeyById.get(folder.parentFolderId) ?? null) : null,
              data: {
                name: folder.name,
            ...
            entries: bookEntries.map((entry) => {
              ...
              return {
                ...
                originalId: entry.id,
                name: entry.name,
                folderKey: entry.folderId ? (folderKeyById.get(entry.folderId) ?? null) : null,
  ...
  const references = options.definition.locations.flatMap((location) =>
    location.lorebookEntryIds.map((entryId): PortableLoreReference => {
      ...
      return {
        ...
        entryKey: options.mode === "map-only" || !entry ? null : (entryKeyById.get(entry.id) ?? null),
...
export function parsePortableLoreBundle(value: unknown): PortableLoreBundle | null {
  if (!isRecord(value) || value.schemaVersion !== 1) return null;
  if (value.mode !== "map-only" && value.mode !== "linked-entries" && value.mode !== "complete-lorebooks") {
    return null;
  }
  if (!Array.isArray(value.books) || !Array.isArray(value.references)) return null;
  ...
  for (const book of books) {
    if (
      ...
      return null;
    if (!isRecord(book.settings) || !Array.isArray(book.folders) || !Array.isArray(book.entries)) return null;
    if (
    ...
    if (book.folders.some((folder) => folder.parentKey && !folderKeys.has(folder.parentKey))) {
      return null;
    }
  ...
  for (const reference of references) {
    ...
    if (reference.entryKey !== null && !entryKeys.has(reference.entryKey)) return null;
  }
  return {
    schemaVersion: 1,
    mode: value.mode,
...
export function planPortableLoreImport(
  ...
  const planEntries = bundle.books.flatMap((book) =>
    book.entries.map((entry): PortableLoreImportPlanEntry => {
      ...
      const contentMatches = (entriesByFingerprint.get(entry.fingerprint) ?? []).filter((candidate) =>
      ...
      return {
        ...
        candidates: matched.map((candidate) => ({
          ...
          lorebookName: lorebookNames.get(candidate.lorebookId) ?? "Unknown lorebook",
  ...
  const planBooks = bundle.books.map((book) => {
    const createdName = nextPortableLorebookName(book.name, mapName, reservedNames);
  ...
  return {
    ...
    books: planBooks,
    entries: planEntries,
    exactMatches: planEntries.filter((entry) => entry.candidates[0]?.reason === "exact-id").length,
    uniqueContentMatches: planEntries.filter(
      (entry) => entry.candidates.length === 1 && entry.candidates[0]?.reason === "content",
    ).length,
    ambiguousMatches: planEntries.filter((entry) => entry.candidates.length > 1).length,
    newEntries: planEntries.filter((entry) => entry.candidates.length === 0).length,
  };
}
...
export function portableLoreImportOutcome(
  ...
  for (const entry of plan.entries) {
    ...
    const candidate = entry.candidates.find((item) => item.entryId === selectedId);
    if (!candidate) {
      unresolvedEntries += 1;
      continue;
    }
  ...
  const createdLorebooks = plan.books
    .filter((book) => book.entryKeys.some((entryKey) => importedEntryKeys.has(entryKey)))
    .map((book) => ({
  ...
  return {
    ...
    reusedLorebooks: [...reusedLorebooks.values()].sort((left, right) => left.name.localeCompare(right.name)),
    createdLorebooks,
    unresolvedEntries,
  };
}

export function portableLoreImportResultMessage(result: PortableLoreImportResult): string {
  const reusedLorebookNames = result.reusedLorebooks.map((book) => `“${book.name}”`).join(", ") || "none";
  const createdLorebookNames = result.createdLorebooks.map((book) => `“${book.name}”`).join(", ") || "none";
...
function foldersInParentOrder(folders: PortableLoreFolder[]): PortableLoreFolder[] {
  const pending = new Map(folders.map((folder) => [folder.key, folder]));
  const ordered: PortableLoreFolder[] = [];
  while (pending.size > 0) {
    const next = [...pending.values()].filter((folder) => !folder.parentKey || !pending.has(folder.parentKey));
    if (next.length === 0) throw new Error("The portable lore bundle contains a folder cycle.");
...
export async function importPortableLoreBundle(options: {
  ...
  const importedDestinations = new Map<string, { lorebookId: string; entryId: string }>();
  let reusedEntries = 0;
  ...
  try {
        ...
        const selectedId =
          entry.candidates.length <= 1
            ...
            : options.ambiguousSelections?.has(entry.entryKey)
              ...
              : undefined;
        if (selectedId === undefined) {
          throw new Error(`Choose how to resolve the duplicate match for “${entry.entryName}”.`);
        }
        if (selectedId !== null) {
          if (!entry.candidates.some((candidate) => candidate.entryId === selectedId)) {
            throw new Error(`The selected lore match for “${entry.entryName}” is no longer available.`);
          }
    ...
    for (const book of options.bundle.books) {
      const entriesToImport = book.entries.filter((entry) => !entryIdMap.has(entry.key));
      if (entriesToImport.length === 0) continue;
      const plannedBook = options.plan.books.find((candidate) => candidate.bookKey === book.key);
      if (!plannedBook) {
        throw new Error(`The import plan is missing the destination for “${book.name}”.`);
      }
      const existingDescription = typeof book.settings.description === "string" ? book.settings.description.trim() : "";
      ...
      const existingTags = Array.isArray(book.settings.tags)
        ? book.settings.tags.filter((tag): tag is string => typeof tag === "string" && tag.trim().length > 0)
        : [];
      const createdBook = await options.api.post<{ id: string }>("/lorebooks", {
        ...
        tags: Array.from(new Set([...existingTags, "World Map", `World Map: ${options.plan.mapName}`])),
        generatedBy: "import",
      ...
      const requiredFolderKeys = new Set(
        entriesToImport.map((entry) => entry.folderKey).filter((key): key is string => Boolean(key)),
      );
      const foldersByKey = new Map(book.folders.map((folder) => [folder.key, folder]));
      ...
      for (const folder of foldersInParentOrder(book.folders.filter((item) => requiredFolderKeys.has(item.key)))) {
        const createdFolder = await options.api.post<{ id: string }>(`/lorebooks/${createdBook.id}/folders`, {
          ...folder.data,
          parentFolderId: folder.parentKey ? (folderIdMap.get(folder.parentKey) ?? null) : null,
        });
      ...
      if (createdEntries.length !== entriesToImport.length) {
        throw new Error(`Only ${createdEntries.length} of ${entriesToImport.length} lore entries were imported.`);
      }
    ...
    const entryKeyByOriginalId = new Map(
      options.bundle.books.flatMap((book) => book.entries.map((entry) => [entry.originalId, entry.key] as const)),
    );
    const relationshipPatches = options.bundle.books.flatMap((book) =>
      book.entries.flatMap((entry) => {
        ...
        const relationships = Object.fromEntries(
          Object.entries(entry.data.relationships).map(([relatedEntryId, relationship]) => {
            const relatedKey = entryKeyByOriginalId.get(relatedEntryId);
            return [relatedKey ? (entryIdMap.get(relatedKey) ?? relatedEntryId) : relatedEntryId, relationship];
          }),
        );
    ...
    for (let index = 0; index < relationshipPatches.length; index += relationshipPatchChunkSize) {
    ...
    return {
      ...
      createdLorebookIds,
      createdLorebooks,
      reusedLorebooks: portableLoreImportOutcome(options.plan, options.strategy, options.ambiguousSelections)
        .reusedLorebooks,
    };
  } catch (error) {
    const cleanupResults = await Promise.allSettled(
      createdLorebookIds.map((lorebookId) => options.api.delete(`/lorebooks/${lorebookId}`)),
    );
    ...
    if (orphanedLorebookIds.length > 0) {
      const importError = error instanceof Error ? error : new Error("The portable lore import failed.");
        ...
        lorebookEntryIds: Array.from(
          new Set(
            location.lorebookEntryIds.map((entryId) => {
              const reference = references.find((candidate) => candidate.originalEntryId === entryId);
              return reference?.entryKey ? (entryIdMap.get(reference.entryKey) ?? entryId) : entryId;
            }),
          ),
        ),
      };
...
export function unresolvedPortableLoreReferences(
  ...
  return bundle.references.filter((reference) => !reference.entryKey || !entryIdMap.has(reference.entryKey));
}

export function portableLoreApproximateBytes(bundle: PortableLoreBundle): number {
