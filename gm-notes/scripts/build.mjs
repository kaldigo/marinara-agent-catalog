...
await fs.mkdir(path.join(out, "tracker-codecs"), { recursive: true });
...
for (const name of ["backfill.js", "routes.js", "index.js"]) {
  const source = (await fs.readFile(path.join(root, "src", "server", name), "utf8"))
    ...
    .replace('../shared/state.js', '../../tracker-codecs/gm-notes.js');
  await fs.writeFile(path.join(out, "src", "server", name), source);
}
await fs.copyFile(path.join(codecRoot, "gm-notes.js"), path.join(out, "tracker-codecs", "gm-notes.js"));
...
for (const file of [
  ...
  path.join(codecRoot, "gm-notes.js"),
