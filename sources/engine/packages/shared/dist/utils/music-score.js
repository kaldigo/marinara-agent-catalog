...
export const MUSIC_INTENSITIES = ["calm", "tense", "intense"];
export const LOCATION_KINDS = ["interior", "exterior", "underground", "urban", "nature"];
const GAME_STATES = new Set(["exploration", "dialogue", "combat", "travel_rest"]);
const MUSIC_GENRE_SET = new Set(MUSIC_GENRES);
const MUSIC_INTENSITY_SET = new Set(MUSIC_INTENSITIES);
const LOCATION_KIND_SET = new Set(LOCATION_KINDS);
const INTENSITY_RANK = {
    calm: 0,
    tense: 1,
    intense: 2,
};
const STATE_DEFAULT_INTENSITY = {
    exploration: "tense",
    dialogue: "calm",
    combat: "intense",
    travel_rest: "calm",
...
export function normalizeMusicGenre(value) {
    ...
    return MUSIC_GENRE_SET.has(normalized) ? normalized : null;
}
export function normalizeMusicIntensity(value) {
    const normalized = normalizeToken(value);
    if (!normalized)
        return null;
    const aliases = {
    ...
    if (aliases[normalized])
        return aliases[normalized];
    return MUSIC_INTENSITY_SET.has(normalized) ? normalized : null;
}
export function normalizeLocationKind(value) {
    const normalized = normalizeToken(value);
    if (!normalized)
        return null;
    const aliases = {
    ...
    if (aliases[normalized])
        return aliases[normalized];
    return LOCATION_KIND_SET.has(normalized) ? normalized : null;
}
function parseMusicTag(tag) {
    const parts = tag.split(":");
    if (parts.length < 5 || parts[0] !== "music")
        return null;
    const state = parts[1];
    if (!state || !GAME_STATES.has(state))
        return null;
...
function inferMusicIntensity(state, weather, timeOfDay) {
...
function musicAccentScore(candidate, weather, timeOfDay) {
    ...
    for (const keyword of keywords) {
        if (candidate.keywords.some((part) => part.includes(keyword) || keyword.includes(part)))
            score += 1;
    }
    return Math.min(score, 2);
}
function scoreStructuredMusic(candidate, desiredGenre, desiredIntensity, hasExactGenre, weather, timeOfDay) {
    let score = 10;
    if (desiredGenre) {
 ...
 */
export function scoreMusic(input) {
    const { state, weather, timeOfDay, currentMusic, recentMusic, availableMusic } = input;
    if (!availableMusic.length)
        return null;
    const desiredGenre = normalizeMusicGenre(input.musicGenre);
    const desiredIntensity = normalizeMusicIntensity(input.musicIntensity) ?? inferMusicIntensity(state, weather, timeOfDay);
    const candidates = availableMusic
        .map((tag) => parseMusicTag(tag))
        .filter((candidate) => !!candidate && candidate.state === state);
...
function inferLocationKindFromBackground(background) {
    const bgLower = (background ?? "").toLowerCase();
    if (!bgLower)
        return null;
    if (/(underground|dungeon|cave|catacomb|crypt|sewer|ruin)/.test(bgLower))
        return "underground";
    if (/(city|street|market|town|village|alley|plaza|urban)/.test(bgLower))
        return "urban";
    if (/(forest|woods|river|lake|mountain|beach|desert|valley|field|nature|swamp)/.test(bgLower))
        return "nature";
...
function ambientKeywordScore(parts, keywords) {
    let score = 0;
    for (const kw of keywords) {
        if (parts.some((part) => part.includes(kw) || kw.includes(part)))
            score++;
    }
    return score;
}
function weatherAllowsStormAudio(weather) {
    const normalized = normalizeToken(weather);
    return normalized === "storm" || normalized === "stormy" || normalized === "thunderstorm";
}
function ambientStormAudioScore(parts, weather) {
...
function ambientLocationScore(parts, locationKind) {
    if (!locationKind)
        return 0;
    const subcategory = parts[1] ?? "";
    if (locationKind === "interior") {
        if (subcategory === "interior" || parts.includes("interior"))
            return 4;
        if (subcategory === "nature" || subcategory === "urban")
            return -2;
    }
    if (locationKind === "underground") {
        if (parts.some((part) => ["dungeon", "cave", "underground"].includes(part)))
            return 4;
    ...
    if (locationKind === "urban") {
        if (subcategory === "urban" || parts.includes("urban") || parts.includes("crowd"))
            return 4;
        if (subcategory === "nature")
            return -2;
    }
    if (locationKind === "nature" || locationKind === "exterior") {
        if (subcategory === "nature" || parts.includes("nature"))
            return 4;
        if (subcategory === "interior")
            return -2;
    }
...
export function scoreAmbient(input) {
    ...
    if (weather)
        keywords.push(...(WEATHER_AMBIENT[weather.toLowerCase()] ?? []));
    if (timeOfDay)
        keywords.push(...(TIME_AMBIENT[timeOfDay.toLowerCase()] ?? []));
    const scored = availableAmbient.map((tag) => {
        const parts = tag
            .toLowerCase()
            .split(/[:\-_]+/)
            .filter((part) => part.length > 1);
        const score = ambientLocationScore(parts, locationKind) +
...
//# sourceMappingURL=music-score.js.map