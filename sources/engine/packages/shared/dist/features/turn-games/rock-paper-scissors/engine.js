...
import { DEFAULT_ROCK_PAPER_SCISSORS_CONFIG, ROCK_PAPER_SCISSORS_LOG_CAP, ROCK_PAPER_SCISSORS_MAX_PLAYERS, ROCK_PAPER_SCISSORS_MIN_PLAYERS, RPS_BEATS, RPS_CHOICES, rockPaperScissorsConfigSchema, } from "./types.js";
// ── small helpers ───────────────────────────────────────────────────────────
const clone = (cloneState);
const nameOf = nameOfSeat;
const record = (state, event) => recordEvent(state, event, ROCK_PAPER_SCISSORS_LOG_CAP);
const setLast = setLastAction;
function opponentOf(state, seatId) {
...
function seatToThrow(state) {
    const [a, b] = state.seatIds;
    if (state.pendingThrows[a] == null)
        return a;
    if (state.pendingThrows[b] == null)
        return b;
    return null;
}
...
// ── prompt builders ─────────────────────────────────────────────────────────
function scoreLine(state, seatId) {
...
function buildParticipantSummary(state, seatId) {
    ...
    if (!state.seatIds.includes(seatId))
        return base;
    const lines = [base, scoreLine(state, seatId)];
    if (state.status !== "finished") {
        if (seatToThrow(state) === seatId)
            lines.push("It is YOUR turn to throw.");
        else if (state.pendingThrows[seatId] != null)
            lines.push("You have already thrown this round — waiting on your opponent.");
    }
    return lines.join("\n");
}
...
// ── the engine object ───────────────────────────────────────────────────────
export const rockPaperScissorsEngine = {
    ...
    legalMoves(state, seatId) {
        if (state.status === "finished" || seatToThrow(state) !== seatId)
            return [];
        return RPS_CHOICES.map((choice) => ({ type: "throw", choice }));
    },
    applyMove(state, seatId, move) {
        ...
        if (!move || move.type !== "throw" || !RPS_CHOICES.includes(move.choice)) {
            return {
                ...
                legalMoves: RPS_CHOICES.map((choice) => ({ type: "throw", choice })),
            };
        }
        const s = clone(state);
        const events = [];
    ...
    describeForModel(state, seatId) {
        const legal = seatToThrow(state) === seatId ? RPS_CHOICES.map((choice) => ({ type: "throw", choice })) : [];
        return {
    ...
    parseToolCall(name, args) {
        if (name !== "rock_paper_scissors_throw")
            return null;
        const raw = typeof args.choice === "string" ? args.choice.trim().toLowerCase() : "";
        const choice = RPS_CHOICES.includes(raw) ? raw : raw;
