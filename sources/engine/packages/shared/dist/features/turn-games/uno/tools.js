...
export const playCardToolManifest = {
    name: "play_card",
    description: "Play one card from your hand onto the discard pile. Must match the active color OR the top card's value, or be a wild. " +
    ...
    parameters: {
        ...
        properties: {
            ...
            declared_color: {
                type: "string",
                description: "Required when playing a wild: choose the color best represented among the colored cards remaining in your hand; do not default to the current color.",
...
export const drawCardToolManifest = {
    name: "draw_card",
    description: "Draw from the draw pile. Use this when you have no playable card, or to take an accumulated +2/+4 penalty.",
...
export const jumpInToolManifest = {
    name: "jump_in",
    description: "Out of turn, play a card identical to the top card (same color AND value) to jump the queue. Only allowed when the jump-in rule is on.",
    parameters: {
        type: "object",
        properties: {
...
export const UNO_TOOL_MANIFESTS = [
...
export const UNO_TOOL_NAMES = UNO_TOOL_MANIFESTS.map((t) => t.name);
