...
import { ballLabel, distance, generateCandidates, ghostBallPoint, inBounds, pocketLabel, railContactPoint, reflectAcrossRail, standardRackPositions, zoneName, } from "./geometry.js";
import { powerToSpeed, simulateShot } from "./physics.js";
import { deterministicRng } from "./rng.js";
import { EIGHTBALL_TOOL_MANIFESTS, parseEightBallToolCall } from "./tools.js";
import { BREAK_NOMINAL_SUCCESS_PCT, BALL_R, CUE_ID, DEFAULT_EIGHTBALL_CONFIG, EIGHTBALL_LOG_CAP, EIGHTBALL_MAX_PLAYERS, EIGHTBALL_MIN_PLAYERS, EIGHT_ID, FOOT_SPOT, KITCHEN_SPOT, POCKETS, POCKET_IDS, SOLID_IDS, STRIPE_IDS, TABLE_HEIGHT, TABLE_WIDTH, clampEightBallConfig, eightBallConfigSchema, } from "./types.js";
// ── small helpers ───────────────────────────────────────────────────────────
...
function record(state, events, event) {
    ...
    if (state.log.length > EIGHTBALL_LOG_CAP)
        state.log.splice(0, state.log.length - EIGHTBALL_LOG_CAP);
}
...
function remainingIdsForGroup(state, group) {
...
function legalTargetIdsFor(state, seatId) {
 ...
 * "at or behind that x".
 */
function isValidCuePlacement(p, onTableBalls, zone) {
    if (!inBounds(p, BALL_R))
        return false;
    for (const b of onTableBalls) {
        if (distance(p, { x: b.x, y: b.y }) < 2 * BALL_R + 0.05)
            return false;
    }
    for (const id of POCKET_IDS) {
        const pocket = POCKETS[id];
        if (distance(p, pocket.pos) < pocket.captureRadius + BALL_R)
            return false;
    }
    if (zone === "kitchen" && p.x > KITCHEN_SPOT.x)
        return false;
    return true;
}
...
function buildCandidateMenu(state, seatId) {
...
function defaultPlacementPoint(state, zone) {
    const onTable = state.balls.filter((b) => b.onTable);
    const candidates = zone === "kitchen"
...
function placementZoneFor(state, seatId) {
    if (state.status !== "active" || state.currentSeatId !== seatId)
        return null;
    if (state.awaitingPlacement)
        return state.placementZone;
    if (state.phase === "break" && state.cuePos !== null)
        return "kitchen";
    return null;
}
...
function buildSimBalls(s) {
    const balls = s.balls.filter((b) => b.onTable).map((b) => ({ id: b.id, x: b.x, y: b.y, vx: 0, vy: 0 }));
    balls.push({ id: CUE_ID, x: s.cuePos.x, y: s.cuePos.y, vx: 0, vy: 0 });
    return balls;
}
function angleDegToVelocity(angleDeg, power) {
...
function applySimToBalls(s, sim) {
...
function isLegalFirstContact(state, seatId, isBreakShot, firstContactBallId) {
...
const JITTER_SIGMA_DEG = { easy: 0.6, medium: 1.4, hard: 2.6, very_hard: 4.2 };
const STYLE_JITTER_MULT = { controlled: 0.7, aggressive: 1.35 };
 ...
 */
const BANK_RAIL_ORDER = ["top", "bottom", "left", "right"];
function findBankAimGeometry(ballPos, pocketPos) {
 ...
 */
function convertMenuToAim(s, candidate, style, isBreakShot) {
    ...
    const jitter = (sigma) => (rng() * 2 - 1) * sigma * mult;
    const angleTo = (target) => (Math.atan2(target.y - effectiveCuePos.y, target.x - effectiveCuePos.x) * 180) / Math.PI;
    if (isBreakShot) {
...
function assignGroupsFromPotted(s, events, seatId, potted) {
    let solids = 0;
    let stripes = 0;
    for (const p of potted) {
        if (SOLID_IDS.includes(p.ballId))
            solids++;
        else if (STRIPE_IDS.includes(p.ballId))
            stripes++;
    }
    let group;
    if (solids !== stripes) {
...
function maybeAnnounceGreatShot(s, events, seatId, reason) {
    announce(s, events, {
        type: "great_shot",
        seatId,
        message: reason === "bank"
 ...
 * resolution path). */
function maybeAnnounceOnTheEight(s, events, seatId) {
...
function describePottedList(potted) {
    if (potted.length === 1)
        return `${ballLabel(potted[0].ballId)} into ${pocketLabel(potted[0].pocketId)}`;
...
function finishRack(s, events, winnerSeatId, wasScratch) {
    ...
    announce(s, events, {
        type: "rack_over",
        seatId: winnerSeatId,
        message: `${nameOf(s, winnerSeatId)} takes rack ${s.rackNumber}${wasScratch ? " — opponent scratched on the 8" : ""}. ` +
            `Score: ${s.rackScore[winnerSeatId]}-${s.rackScore[loserSeatId] ?? 0}.`,
    });
}
function freshRackBalls() {
    return standardRackPositions(FOOT_SPOT).map((b) => ({ id: b.id, x: b.pos.x, y: b.pos.y, onTable: true }));
}
function startNewRack(s, events) {
    s.rackNumber += 1;
    s.phase = "break";
    s.status = "active";
 ...
 */
function resolveShot(s, seatId, isBreakShot, moveKind, sim, events, candidate) {
    const opp = otherSeat(s, seatId);
    ...
    if (foul) {
        ...
        s.currentSeatId = opp;
        if (isBreakShot)
            s.phase = "play"; // the break always resolves the phase, foul or not
    ...
    if (pottedOwn.length > 0) {
        ...
        if (pottedOwn.length >= 2)
            maybeAnnounceGreatShot(s, events, seatId, "multi");
        else if (bankSuccess)
            maybeAnnounceGreatShot(s, events, seatId, "bank");
...
function buildBoardSummary(state, seatId) {
    ...
    if (state.phase === "break") {
        ...
        if (!myGroup) {
            ...
            const oppGroup = myGroup === "solids" ? "stripes" : "solids";
        ...
        if (state.ballInHandFor === seatId)
            lines.push("You have BALL IN HAND — your shot choice places it for you.");
    }
...
function buildInstructions(state, seatId, candidates) {
...
function buildSpectatorSummary(state) {
    ...
    if (state.status === "rack_over")
        lines.push("The rack just ended — waiting on the next rack.");
    else if (state.currentSeatId)
        lines.push(`It's currently ${nameOf(state, state.currentSeatId)}'s shot.`);
...
function buildParticipantSummary(state, seatId) {
    ...
    if (state.status !== "finished") {
        ...
        if (group)
            lines.push(`Your group: ${group}, ${remainingIdsForGroup(state, group).length} left.`);
...
const TIER_ORDER = { easy: 0, medium: 1, hard: 2, very_hard: 3 };
function pickFallback(state, seatId) {
    if (state.status === "rack_over")
        return { kind: "next_rack" };
    const legal = legalMovesFor(state, seatId);
    const menuMoves = legal.filter((m) => m.kind === "menu");
    if (menuMoves.length === 0) {
        const placeMove = legal.find((m) => m.kind === "place");
        if (placeMove)
            return placeMove;
...
export const eightBallEngine = {
    ...
    normalizeConfig(config) {
        ...
        const merged = direct.success
            ? direct.data
            : { ...DEFAULT_EIGHTBALL_CONFIG, ...(config && typeof config === "object" ? config : {}) };
        return clampEightBallConfig(merged);
    },
    setup(config, seatsIn, seed) {
        ...
        const breakerSeatId = config.humanBreaks === "you"
            ? humanSeat.seatId
    ...
    applyMove(state, seatId, move) {
        ...
        if (move?.kind === "place") {
            const zone = placementZoneFor(state, seatId);
            if (!zone)
                return fail("You don't have ball in hand right now.", legalMovesFor(state, seatId));
            ...
            const onTable = state.balls.filter((b) => b.onTable);
            if (!isValidCuePlacement(p, onTable, zone)) {
                return fail("That placement isn't legal — off the table, overlapping a ball, inside a pocket's zone, or outside the kitchen.", legalMovesFor(state, seatId));
            }
            const s = clone(state);
            s.cuePos = p;
        ...
        if (move?.kind === "menu") {
            ...
            const style = move.style === "aggressive" ? "aggressive" : "controlled";
            const isBreakShot = state.phase === "break";
            const s = clone(state);
    ...
    publicView(state, viewerSeatId) {
        const seats = state.seatOrder.map((id) => {
        ...
        const balls = state.balls.map((b) => ({ id: b.id, x: b.x, y: b.y, pocketed: !b.onTable }));
        if (state.cuePos)
            balls.push({ id: CUE_ID, x: state.cuePos.x, y: state.cuePos.y, pocketed: false });
        balls.sort((a, b) => a.id - b.id);
        const pocketedByGroup = { solids: 0, stripes: 0 };
        for (const b of state.balls) {
            if (b.onTable)
                continue;
            if (SOLID_IDS.includes(b.id))
                pocketedByGroup.solids += 1;
            else if (STRIPE_IDS.includes(b.id))
                pocketedByGroup.stripes += 1;
        }
        let yourShots = null;
