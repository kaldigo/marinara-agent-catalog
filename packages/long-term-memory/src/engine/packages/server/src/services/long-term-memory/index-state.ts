...
import { ltmIndexStateSchema, type LtmIndexState, type LtmNote } from "../../../../shared/src/features/agents/long-term-memory/schema.js";
...
type LtmNoteSummary = { version: 1; lastMutationId?: string; total: number; byType: Record<string, number>; byStatus: Record<string, number> };
...
function isCountBucket(value: unknown): value is Record<string, number> {
  return Boolean(value && typeof value === "object" && !Array.isArray(value) && Object.values(value).every((count) => Number.isInteger(count) && (count as number) >= 0));
}
function isNoteSummary(value: unknown): value is LtmNoteSummary {
  if (!value || typeof value !== "object") return false;
  const summary = value as Record<string, unknown>;
  return summary.version === 1 && Number.isInteger(summary.total) && summary.total >= 0 &&
    isCountBucket(summary.byType) && isCountBucket(summary.byStatus);
}
function addNote(summary: LtmNoteSummary, note: LtmNote, delta: 1 | -1) {
  summary.total += delta;
  for (const [key, value] of [[note.type, summary.byType], [note.status, summary.byStatus]] as const) {
...
export async function rebuildLtmNoteSummary(root: string) {
  ...
  for (const folder of LTM_VAULT_FOLDERS) {
    for (const entry of await readdir(safeJoin(dirs.vault, folder), { withFileTypes: true })) {
      ...
      const note = parseStoredLtmNote(JSON.parse(await readFile(safeJoin(dirs.vault, `${folder}/${entry.name}`), "utf8")));
...
export async function readLtmNoteSummary(root: string) {
  ...
  if (isNoteSummary(summary))
    return summary as LtmNoteSummary;
  return rebuildLtmNoteSummary(root);
}
export async function updateLtmNoteSummary(root: string, mutationId: string, changes: Array<{ before: unknown; after: unknown }>) {
  const stored = await readJsonFile<unknown>(noteSummaryPath(root), null);
  if (!isNoteSummary(stored))
    return rebuildLtmNoteSummary(root);
...
function parseNote(value: unknown) {
  ...
  try { return parseStoredLtmNote(value); } catch { return null; }
}
...
export function markLtmIndexesBuilding(root: string) { return update(root, (state) => ({ ...state, revision: state.revision + 1, dirty: true, dirtyAt: state.dirtyAt ?? nowIso(), rebuildState: "building", rebuildStartedAt: nowIso(), rebuildCompletedAt: undefined, error: undefined })); }
export function markLtmIndexesClean(root:string){return update(root,(state)=>({...state,revision:state.revision + 1,dirty:false,dirtyAt:undefined,rebuildState:"idle",rebuildCompletedAt:nowIso(),error:undefined}));}
export function markLtmIndexesFailed(root: string, error: unknown) { return update(root, (state) => ({ ...state, revision: state.revision + 1, dirty: true, rebuildState: "failed", rebuildCompletedAt: nowIso(), error: error instanceof Error ? error.message.slice(0, 2_000) : "Recall index rebuild failed." })); }
