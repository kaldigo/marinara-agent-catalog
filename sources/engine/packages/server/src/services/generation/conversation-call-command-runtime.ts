...
function isConversationCommandEnabled(metadata: Record<string, unknown>, key: string): boolean {
  const toggles = metadata.conversationCommandToggles;
  return !toggles || typeof toggles !== "object" || Array.isArray(toggles)
    ? true
    : (toggles as Record<string, unknown>)[key] !== false;
}
...
export async function handleConversationCallCommand(args: {
  ...
  const activeAgentIds = Array.isArray(freshMeta.activeAgentIds) ? freshMeta.activeAgentIds : [];
  if (freshMeta.enableAgents !== true || !activeAgentIds.includes("conversation-calls")) {
    logger.debug("[commands] Ignored call command because the Conversation Calls agent is inactive");
    return true;
  }
