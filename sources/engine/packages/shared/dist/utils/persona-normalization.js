...
export function normalizePersonaStringArray(value) {
...
export function normalizeTrackerCardColorConfig(value) {
    const config = decodeJsonValue(value);
    if (!isRecord(config))
        return { mode: "chat" };
    const mode = config.mode === "default" || config.mode === "chat" || config.mode === "custom" ? config.mode : "chat";
    const result = { ...config, mode };
    for (const key of ["displayEnabled", "accentEnabled", "surfaceEnabled"]) {
        if (typeof config[key] === "boolean")
            result[key] = config[key];
        else
            delete result[key];
    }
    for (const key of ["nameColor", "dialogueColor", "boxColor"]) {
    ...
    if (portraitFocusY === undefined)
        delete result.portraitFocusY;
    else
        result.portraitFocusY = portraitFocusY;
    const portraitZoom = clampedPortraitZoom(config.portraitZoom);
    if (portraitZoom === undefined)
        delete result.portraitZoom;
    else
        result.portraitZoom = portraitZoom;
    const portraitStageBackground = config.portraitStageBackground === "ambient" ||
        ...
        : undefined;
    if (portraitStageBackground === undefined)
        delete result.portraitStageBackground;
    else
        result.portraitStageBackground = portraitStageBackground;
...
function normalizeStatBar(value) {
    if (!isRecord(value) ||
        ...
        return null;
    return { ...value, name: value.name, value: value.value, max: value.max, color: value.color };
}
function normalizeRpgValueRange(value, max) {
...
function normalizeRpgStats(value) {
    if (!isRecord(value) ||
        ...
        return undefined;
    const attributes = value.attributes.flatMap((attribute) => isRecord(attribute) && typeof attribute.name === "string" && Number.isFinite(attribute.value)
    ...
    const pools = Array.isArray(value.pools)
        ...
        : undefined;
    const hp = normalizeRpgValueRange(value.hp.value, value.hp.max);
    const result = {
        ...value,
...
export function normalizePersonaStats(value) {
    const config = decodeJsonValue(value);
    if (!isRecord(config) || typeof config.enabled !== "boolean" || !Array.isArray(config.bars))
        return undefined;
    ...
    const result = { ...config, enabled: config.enabled, bars };
    if (rpgStats)
...
export function normalizeConvoBehavior(value) {
    const config = decodeJsonValue(value);
    if (!isRecord(config) || typeof config.instruction !== "string")
        return undefined;
    ...
    return { ...config, instruction: config.instruction, insertionStrategy };
}
