...
async function main() {
  ...
  const manifest = {
    ...
    config: {
      name: "Presence",
      description: "Tracks per-message character presence and temporarily filters prompt history during generation.",
      enabled: true,
      jsPath: "presence.js",
    },
  };

  await fs.writeFile(path.join(packageDir, "presence.js"), js);
  await writeJson(path.join(packageDir, "manifest.json"), manifest);
  await writeJson(path.join(distRoot, "presence.json"), {
    kind: "marinara.extension",
  ...
  console.log("Built Marinara Presence");
