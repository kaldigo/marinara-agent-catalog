...
export const eightBallActionToolManifest = {
    ...
    description: "Take your 8-ball shot. Copy `shotId` EXACTLY from the shot menu you were given " +
...
/** All 8-ball tools. */
export const EIGHTBALL_TOOL_MANIFESTS = [eightBallActionToolManifest];
...
export function parseEightBallToolCall(name, args) {
    if (name !== "eightball_action")
        return null;
    ...
    const style = rawStyle === "aggressive" ? "aggressive" : "controlled";
    if (action === "break")
        return { kind: "menu", shotId: "break", style };
    if (action === "shoot") {
