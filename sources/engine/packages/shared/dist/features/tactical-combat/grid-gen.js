...
function envProfile(environment) {
    return environment ? ENV_PROFILES[environment] : DEFAULT_PROFILE;
}
const BLOB_TERRAINS = ["forest", "mountain", "ruin", "water", "wall"];
...
function inSpawnZone(grid, x) {
    return x < SPAWN_COLS || x >= grid.width - SPAWN_COLS;
}
/** Grow a blob of `terrain` around a center via a bounded random walk. */
function growBlob(grid, cx, cy, size, terrain, rng) {
    let x = cx;
    let y = cy;
    for (let i = 0; i < size; i++) {
        if (!inSpawnZone(grid, x))
            set(grid, x, y, terrain);
    ...
    while (queue.length) {
        ...
        for (const [dx, dy] of [
            ...
            if (nx < 0 || ny < 0 || nx >= grid.width || ny >= grid.height)
                continue;
            if (seen.has(key) || isImpassable(grid, nx, ny))
                continue;
            seen.add(key);
...
function claimNear(grid, occupied, target) {
    ...
    while (queue.length) {
        ...
        for (const [dx, dy] of [
            ...
            const ny = cur.y + dy;
            const nkey = `${nx},${ny}`;
            if (!inBounds(grid, nx, ny) || seen.has(nkey))
                continue;
            seen.add(nkey);
            queue.push({ x: nx, y: ny });
        }
...
function columnTargets(grid, cols, count) {
    const out = [];
...
function rowTargets(grid, y, count) {
    const out = [];
    for (let i = 0; i < count; i++) {
...
function perimeterTargets(grid, count) {
    const W = grid.width;
    const H = grid.height;
    const edges = [
...
function defensiveAdjacency(grid, x, y) {
    let score = 0;
    for (const [dx, dy] of [
        [1, 0],
        [-1, 0],
 ...
 */
function formationTargets(grid, formation, partyCount, enemyCount, rng) {
    const W = grid.width;
    const H = grid.height;
    ...
    switch (formation) {
        ...
        case "skirmish": {
            ...
            const scatter = (center, count) => Array.from({ length: count }, () => ({
